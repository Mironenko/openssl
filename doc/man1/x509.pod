...
Option which determines how extensions in certificate requests (if B<-req>
option present) or certificate (if B<-x509toreq> option present) should be
handled. If set to B<none> or this option is not present then extensions are
ignored and not copied to the certificate or certificate request.
If set to B<copy> then any extensions present in the request or certificate
that are not already present are copied to the certificate or request.
If set to B<copyall> then all extensions in the request or certificate
are copied to the certificate or request: if the extension is already present
in the certificate or request it is deleted first.
